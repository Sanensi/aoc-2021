...
int planCourse(const vector<InputType>& input) {
  ...
  for (const auto& command: input) {
    if (command.first == "forward") horizontal_position += command.second;
    else if (command.first == "down") depth += command.second;
    else if (command.first == "up") depth -= command.second;
